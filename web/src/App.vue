...
async function loadPlanning(showBusy=true){if(!selected.value)return;if(showBusy)planningBusy.value=true;try{planning.value=await api(`/projects/${selected.value}/planning`);const connection=planning.value?.clickup;if(connection)clickupForm.value={workspace_id:connection.workspace_id||"",list_id:connection.list_id||"",tag_name:connection.tag_name||selected.value,token:"",enabled:connection.enabled!==0,status_mapping:{...(connection.status_mapping||{})}}}catch(e:any){error.value=e.message}finally{if(showBusy)planningBusy.value=false}}
