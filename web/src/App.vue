...
async function loadReportHistory(){try{managementReports.value=await api<any[]>('/management/reports');reportComparison.value=managementReports.value.length>1?await api<any>('/management/reports/compare'):null}catch{reportComparison.value=null}}
  ...
  <div class="shell">
    ...
    <main id="main-content" tabindex="-1">
      ...
      <section v-if="activeView === 'reports'" class="reports-screen">
        ...
        <section v-if="reportComparison" class="panel report-comparison"><div class="panel-head"><div><p class="eyebrow">Report evolution</p><h2>Latest report comparison</h2></div><span>{{ date(reportComparison.before.generated_at) }} → {{ date(reportComparison.after.generated_at) }}</span></div><div class="comparison-deltas"><article v-for="(value,key) in reportComparison.delta" :key="key"><span>{{ String(key).replaceAll('_',' ') }}</span><strong :class="{warn:Number(value)<0}">{{ Number(value)>0?'+':'' }}{{ value }}</strong></article></div><div class="comparison-projects"><article v-for="project in reportComparison.projects" :key="project.slug"><strong>{{ project.name }}</strong><span>{{ project.progress>=0?'+':'' }}{{ project.progress }} progress</span><span>{{ project.evidence>=0?'+':'' }}{{ project.evidence }} proofs</span><span>{{ project.blockers>=0?'+':'' }}{{ project.blockers }} blockers</span></article></div></section>
