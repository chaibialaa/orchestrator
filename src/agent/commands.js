  ...
  async plan(...argv) {
    ...
    do {
      ...
      const memoryInstruction = recalibration ? [
        ...
        ' "criterion":"the REWRITTEN criterion — or null if it should stay exactly as it is.',
        '   Never paraphrase it back: a summary applied over the original silently deletes',
        '   whatever the summary left out.",',
        ' "contradiction":["the two requirements that cannot both hold"],',
        ' "decision_needed":"the question only a person can settle, in ONE sentence, or null",',
        ' "options":[{"label":"four words naming the branch",',
        '             "gives_up":"what is abandoned by taking it — the price, stated plainly",',
        '             "then":"what happens next if it is taken"}],',
        ' "steps":[{"title":"…","proof_spec":"…","blast_radius":"feature"}]}',
        'Leave `steps` empty unless splitting is genuinely the answer.',
        '',
        'About `options`, which matter more than the prose: when a person has to choose,',
        'give them TWO or THREE, never more, and make each one a real branch rather than a',
        'restatement. `label` is what a button would say. `gives_up` is the price — an option',
        'that costs nothing is not an option, it is a preference. `then` is what the next pass',
        'would do. Somebody must be able to choose by reading only these three fields: the',
        'reasoning above them is there to be checked, not to be summarised by the reader.',
      ].filter(Boolean).join('\n') : [
