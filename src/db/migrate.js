...
const addEvidenceVerificationColumns=db=>{if(!hasTable(db,'evidence_manifests'))return;const columns=new Set(db.prepare('PRAGMA table_info(evidence_manifests)').all().map(row=>row.name));if(!columns.has('verification'))db.exec("ALTER TABLE evidence_manifests ADD COLUMN verification TEXT CHECK(verification IS NULL OR verification IN ('verified','modified','missing','unverified','not_local'));");if(!columns.has('verified_at'))db.exec('ALTER TABLE evidence_manifests ADD COLUMN verified_at TEXT;');if(!columns.has('actual_sha256'))db.exec('ALTER TABLE evidence_manifests ADD COLUMN actual_sha256 TEXT;');if(!columns.has('actual_bytes'))db.exec('ALTER TABLE evidence_manifests ADD COLUMN actual_bytes INTEGER;')}
...
export function migrate(path = dbPath()) {
  ...
  if (current === 22) { db.close(); return { migrated: false, version: 22 } }
  if (current === 21) { addEvidenceVerificationColumns(db);db.prepare('INSERT INTO schema_migrations(version,checksum) VALUES(22,?)').run(schemaChecksum());db.close();return { migrated: true, version: 22, counts: {} } }
  if (current === 20) { db.exec(schemaSql());addEvidenceVerificationColumns(db);for(const version of [21,22])db.prepare('INSERT INTO schema_migrations(version,checksum) VALUES(?,?)').run(version,schemaChecksum());db.close();return { migrated: true, version: 22, counts: {} } }
  if (current === 19) { db.exec(schemaSql());addEvidenceVerificationColumns(db);for(const version of [20,21,22])db.prepare('INSERT INTO schema_migrations(version,checksum) VALUES(?,?)').run(version,schemaChecksum());db.close();return { migrated: true, version: 22, counts: {} } }
  if (current === 18) { db.exec(schemaSql());addEvidenceVerificationColumns(db);for(const version of [19,20,21,22])db.prepare('INSERT INTO schema_migrations(version,checksum) VALUES(?,?)').run(version,schemaChecksum());db.close();return { migrated: true, version: 22, counts: {} } }
  if (current === 17) { db.exec(schemaSql());addEvidenceVerificationColumns(db);const workspace=initializeWorkspace(db);initializeTeamRecords(db,workspace);for(const version of [18,19,20,21,22])db.prepare('INSERT INTO schema_migrations(version,checksum) VALUES(?,?)').run(version,schemaChecksum());db.close();return { migrated: true, version: 22, counts: {} } }
  if (current === 16) {
    db.exec(schemaSql())
  ...
  const tx = db.transaction(() => {
    ...
    if (!legacy) {
      ...
      db.prepare('INSERT INTO schema_migrations(version,checksum) VALUES(19,?)').run(schemaChecksum())
      db.prepare('INSERT INTO schema_migrations(version,checksum) VALUES(20,?)').run(schemaChecksum())
      db.prepare('INSERT INTO schema_migrations(version,checksum) VALUES(21,?)').run(schemaChecksum())
      db.prepare('INSERT INTO schema_migrations(version,checksum) VALUES(22,?)').run(schemaChecksum())
  ...
  return { migrated: true, version: 22, counts }
}
