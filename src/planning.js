...
export function clickupConnection(project,origin){const row=base().prepare('SELECT * FROM clickup_connections WHERE project_id=?').get(project.id),oauth=clickupOauthConfig(origin),statuses=base().prepare("SELECT COALESCE(ticket_status,'unknown') status,count(*) count FROM clickup_ticket_links WHERE project_id=? GROUP BY COALESCE(ticket_status,'unknown') ORDER BY count DESC").all(project.id),proposalCounts=base().prepare('SELECT status,count(*) count FROM work_proposals WHERE project_id=? GROUP BY status').all(project.id);return row?{...row,status_mapping:statusMapping(row.status_mapping),tag_name:row.tag_name||project.slug,connected:Boolean(row.token||process.env.CLICKUP_API_TOKEN),token:row.token?'configured':process.env.CLICKUP_API_TOKEN?'environment':null,oauth,progress:{tickets:statuses.reduce((sum,item)=>sum+item.count,0),statuses,proposals:proposalCounts}}:{connected:false,status_mapping:{},tag_name:project.slug,token:process.env.CLICKUP_API_TOKEN?'environment':null,oauth,progress:{tickets:0,statuses:[],proposals:proposalCounts}}}

export async function clickupResources(project){const connection=base().prepare('SELECT * FROM clickup_connections WHERE project_id=?').get(project.id),token=connection&&clickupToken(connection);if(!token)throw new Error('Connect ClickUp first.');const teams=(await clickup('/team',token)).teams||[],workspaces=teams.map(team=>({id:String(team.id),name:team.name})),lists=[];for(const team of teams){const spaces=(await clickup(`/team/${encodeURIComponent(team.id)}/space?archived=false`,token)).spaces||[];for(const space of spaces){const direct=(await clickup(`/space/${encodeURIComponent(space.id)}/list?archived=false`,token)).lists||[];for(const list of direct)lists.push({id:String(list.id),name:list.name,statuses:list.statuses||[],workspace_id:String(team.id),workspace_name:team.name,space_name:space.name,folder_name:null});const folders=(await clickup(`/space/${encodeURIComponent(space.id)}/folder?archived=false`,token)).folders||[];for(const folder of folders)for(const list of folder.lists||[])lists.push({id:String(list.id),name:list.name,statuses:list.statuses||[],workspace_id:String(team.id),workspace_name:team.name,space_name:space.name,folder_name:folder.name})}}return{workspaces,lists}}
export async function clickupListStatuses(project,listId){const connection=base().prepare('SELECT * FROM clickup_connections WHERE project_id=?').get(project.id),token=connection&&clickupToken(connection);if(!token)throw new Error('Connect ClickUp first.');const list=await clickup(`/list/${encodeURIComponent(clean(listId))}`,token);return{list_id:String(list.id),statuses:(list.statuses||[]).map(row=>({status:row.status,type:row.type,orderindex:row.orderindex}))}}

export function saveClickupConnection(project,input){const listId=clean(input.list_id),tagName=clean(input.tag_name)||project.slug,mapping=JSON.stringify(statusMapping(JSON.stringify(input.status_mapping||{})));if(!listId)throw new Error('ClickUp list_id is required.');base().prepare(`INSERT INTO clickup_connections(project_id,workspace_id,list_id,tag_name,status_mapping,token,enabled,updated_at)
  VALUES(?,?,?,?,?,?,?,?) ON CONFLICT(project_id) DO UPDATE SET workspace_id=excluded.workspace_id,list_id=excluded.list_id,tag_name=excluded.tag_name,status_mapping=excluded.status_mapping,token=COALESCE(excluded.token,clickup_connections.token),enabled=excluded.enabled,updated_at=excluded.updated_at`).run(project.id,clean(input.workspace_id)||null,listId,tagName,mapping,clean(input.token)||null,input.enabled===false?0:1,nowStamp());return clickupConnection(project)}
