...
export async function clickupExistingTasks(project,query=''){const token=clickupToken(),connection=base().prepare('SELECT * FROM clickup_connections WHERE project_id=?').get(project.id);if(!token)throw new Error('Connect ClickUp first.');if(!connection?.list_id)throw new Error('Choose this project ClickUp list first.');const wanted=clean(query).toLowerCase(),tasks=[];for(let page=0;page<20;page++){const response=await clickup(`/list/${encodeURIComponent(connection.list_id)}/task?include_closed=true&subtasks=true&page=${page}`,token),rows=response.tasks||[];for(const task of rows)if(!wanted||`${task.id} ${task.name} ${task.status?.status||''}`.toLowerCase().includes(wanted))tasks.push({provider:'clickup',external_id:String(task.id),title:task.name,status:task.status?.status||null,url:task.url||null,assignees:(task.assignees||[]).map(person=>({id:String(person.id),name:person.username||person.email,email:person.email||null})),updated_at:task.date_updated?new Date(Number(task.date_updated)).toISOString():null});if(rows.length<100||tasks.length>=100)break}return{project:project.slug,list_id:connection.list_id,query:clean(query),tasks:tasks.slice(0,100),execution:false}}

export function saveClickupConnection(project,input){const listId=clean(input.list_id),tagName=clean(input.tag_name)||project.slug,tagColor=validColor(input.tag_color,defaultProjectTagColor(project)),mapping=JSON.stringify(statusMapping(JSON.stringify(input.status_mapping||{})));if(!listId)throw new Error('ClickUp list_id is required.');if(clean(input.token))saveClickupAccount(input.token,clean(input.workspace_id)||null);base().prepare(`INSERT INTO clickup_connections(project_id,workspace_id,list_id,tag_name,tag_color,status_mapping,token,enabled,updated_at)
  VALUES(?,?,?,?,?,?,NULL,?,?) ON CONFLICT(project_id) DO UPDATE SET workspace_id=excluded.workspace_id,list_id=excluded.list_id,tag_name=excluded.tag_name,tag_color=excluded.tag_color,status_mapping=excluded.status_mapping,token=NULL,enabled=excluded.enabled,updated_at=excluded.updated_at`).run(project.id,clean(input.workspace_id)||null,listId,tagName,tagColor,mapping,input.enabled===false?0:1,nowStamp());return clickupConnection(project)}
...
export async function syncClickup(project,onProgress=()=>{},{trigger='manual'}={}){
  ...
  try{
    ...
    const list=await clickup(`/list/${encodeURIComponent(connection.list_id)}`,token),spaceId=list.space?.id,tagName=connection.tag_name||project.slug,tagColor=validColor(connection.tag_color,defaultProjectTagColor(project)),listStatuses=list.statuses||[],customStatuses=statusMapping(connection.status_mapping)
    onProgress(15,'Checking project tag')
    if(spaceId){const tags=await clickup(`/space/${encodeURIComponent(spaceId)}/tag`,token),existing=(tags.tags||[]).find(tag=>String(tag.name).toLowerCase()===tagName.toLowerCase()),tag={name:tagName,tag_fg:'#ffffff',tag_bg:tagColor};if(!existing)await clickup(`/space/${encodeURIComponent(spaceId)}/tag`,token,{method:'POST',body:JSON.stringify({tag})});else if(String(existing.tag_bg||'').toLowerCase()!==tagColor)await clickup(`/space/${encodeURIComponent(spaceId)}/tag/${encodeURIComponent(tagName)}`,token,{method:'PUT',body:JSON.stringify({tag})})}
