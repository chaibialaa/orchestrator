...
export async function syncClickup(project,onProgress=()=>{},{trigger='manual'}={}){
  ...
  try{
    ...
    const list=await clickup(`/list/${encodeURIComponent(connection.list_id)}`,token),spaceId=list.space?.id,tagName=connection.tag_name||project.slug,listStatuses=list.statuses||[]
