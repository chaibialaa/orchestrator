...
function clickupToken(connection){return process.env.CLICKUP_API_TOKEN||connection.token}
...
async function clickupAttachment(taskId,token,path){for(let attempt=0;attempt<3;attempt++){const form=new FormData();form.append('attachment',new Blob([readFileSync(path)]),basename(path));const response=await fetch(`https://api.clickup.com/api/v2/task/${encodeURIComponent(taskId)}/attachment`,{method:'POST',headers:{Authorization:token},body:form});if(response.ok)return;if(response.status===429&&attempt<2){await wait(Math.min(60000,Math.max(2000,Number(response.headers.get('retry-after')||0)*1000)));continue}throw new Error(`ClickUp attachment ${response.status}`)}}
...
export async function connectClickupOauth(project,token){const teams=await clickup('/team',token),workspace=teams.teams?.[0];base().prepare(`INSERT INTO clickup_connections(project_id,workspace_id,list_id,token,enabled,last_status,last_detail,updated_at) VALUES(?,?,?,?,1,'ok',?,?) ON CONFLICT(project_id) DO UPDATE SET workspace_id=excluded.workspace_id,token=excluded.token,enabled=1,last_status='ok',last_detail=excluded.last_detail,updated_at=excluded.updated_at`).run(project.id,workspace?.id||null,'',token,'Connected with ClickUp. Choose a destination list.',nowStamp());return clickupConnection(project)}
export function clickupConnection(project,origin){const row=base().prepare('SELECT * FROM clickup_connections WHERE project_id=?').get(project.id),oauth=clickupOauthConfig(origin),statuses=base().prepare("SELECT COALESCE(ticket_status,'unknown') status,count(*) count FROM clickup_ticket_links WHERE project_id=? GROUP BY COALESCE(ticket_status,'unknown') ORDER BY count DESC").all(project.id),proposalCounts=base().prepare('SELECT status,count(*) count FROM work_proposals WHERE project_id=? GROUP BY status').all(project.id);return row?{...row,tag_name:row.tag_name||project.slug,connected:Boolean(row.token||process.env.CLICKUP_API_TOKEN),token:row.token?'configured':process.env.CLICKUP_API_TOKEN?'environment':null,oauth,progress:{tickets:statuses.reduce((sum,item)=>sum+item.count,0),statuses,proposals:proposalCounts}}:{connected:false,tag_name:project.slug,token:process.env.CLICKUP_API_TOKEN?'environment':null,oauth,progress:{tickets:0,statuses:[],proposals:proposalCounts}}}

export async function clickupResources(project){const connection=base().prepare('SELECT * FROM clickup_connections WHERE project_id=?').get(project.id),token=connection&&clickupToken(connection);if(!token)throw new Error('Connect ClickUp first.');const teams=(await clickup('/team',token)).teams||[],workspaces=teams.map(team=>({id:String(team.id),name:team.name})),lists=[];for(const team of teams){const spaces=(await clickup(`/team/${encodeURIComponent(team.id)}/space?archived=false`,token)).spaces||[];for(const space of spaces){const direct=(await clickup(`/space/${encodeURIComponent(space.id)}/list?archived=false`,token)).lists||[];for(const list of direct)lists.push({id:String(list.id),name:list.name,workspace_id:String(team.id),workspace_name:team.name,space_name:space.name,folder_name:null});const folders=(await clickup(`/space/${encodeURIComponent(space.id)}/folder?archived=false`,token)).folders||[];for(const folder of folders)for(const list of folder.lists||[])lists.push({id:String(list.id),name:list.name,workspace_id:String(team.id),workspace_name:team.name,space_name:space.name,folder_name:folder.name})}}return{workspaces,lists}}

export function saveClickupConnection(project,input){const listId=clean(input.list_id),tagName=clean(input.tag_name)||project.slug;if(!listId)throw new Error('ClickUp list_id is required.');base().prepare(`INSERT INTO clickup_connections(project_id,workspace_id,list_id,tag_name,token,enabled,updated_at)
  VALUES(?,?,?,?,?,?,?) ON CONFLICT(project_id) DO UPDATE SET workspace_id=excluded.workspace_id,list_id=excluded.list_id,tag_name=excluded.tag_name,token=COALESCE(excluded.token,clickup_connections.token),enabled=excluded.enabled,updated_at=excluded.updated_at`).run(project.id,clean(input.workspace_id)||null,listId,tagName,clean(input.token)||null,input.enabled===false?0:1,nowStamp());return clickupConnection(project)}

export async function syncClickup(project,onProgress=()=>{},{trigger='manual'}={}){
  const db=base(),connection=db.prepare('SELECT * FROM clickup_connections WHERE project_id=? AND enabled=1').get(project.id);if(!connection)throw new Error('ClickUp is not enabled for this project.');if(!connection.list_id)throw new Error('Choose a ClickUp destination list first.');const token=clickupToken(connection);if(!token)throw new Error('Connect ClickUp or save a ClickUp token.')
  ...
  try{
    ...
    const list=await clickup(`/list/${encodeURIComponent(connection.list_id)}`,token),spaceId=list.space?.id,tagName=connection.tag_name||project.slug
    ...
    for(const proposal of proposalRows){proposalIndex++;onProgress(20+Math.round(45*proposalIndex/Math.max(1,proposalRows.length)),`Synchronizing registry ${proposalIndex}/${proposalRows.length}`);const fp=`proposal:${proposal.fingerprint}`,existing=db.prepare('SELECT * FROM clickup_ticket_links WHERE fingerprint=?').get(fp),objective=proposal.objective_id?db.prepare('SELECT id,parent_id FROM objectives WHERE id=?').get(proposal.objective_id):null,proofScope=[objective?.id,objective?.parent_id].filter(Boolean),proofs=proofScope.length?db.prepare(`SELECT e.label,e.sha256,e.locator,e.locator_kind,e.bytes FROM evidence_manifests e WHERE e.project_id=? AND e.objective_id IN (${proofScope.map(()=>'?').join(',')}) ORDER BY e.created_at DESC LIMIT 20`).all(project.id,...proofScope):db.prepare('SELECT label,sha256,locator,locator_kind,bytes FROM evidence_manifests WHERE project_id=? ORDER BY created_at DESC LIMIT 20').all(project.id),stateTag=lifecycle[proposal.status]||'orchestrator-backlog'
      ...
      const syncHash=digest(JSON.stringify({title:proposal.title,description,stateTag,tagName}));let task
