...
const state={active:false,started_at:null,last_change_at:null,errors:[],timer:null}
...
export function passiveFollowerState(){const {timer:_timer,...visible}=state;return{...visible,projects:[...followers.values()].map(row=>({slug:row.project.slug,root:row.root,pending:row.paths.size,last_change_at:row.last_change_at||null}))}}
export function startPassiveFollowers({onBatch,onPlan,debounceMs=1500,rescanMs=30000}={}){
  ...
  reconcile();state.timer=setInterval(reconcile,rescanMs);state.timer.unref();return passiveFollowerState()
}
export function stopPassiveFollowers(){if(state.timer)clearInterval(state.timer);for(const row of followers.values()){row.watcher.close();clearTimeout(row.timer)}followers.clear();state.active=false;state.timer=null}
